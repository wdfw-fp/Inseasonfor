...
model_results2 |> dplyr::filter(date==forecastdate,mod_type%in%c("ARIMA","DLM")) |> dplyr::select(Model=mod_type,coef_intercept, coef_log_lag_jack:coef_cnt_by_flow,covar_log_lag_jack:covar_cnt_by_flow,effect_log_lag_jack:effect_cnt_by_flow) |>   
  gt::gt() |>
...
model_results2 |> dplyr::filter(date==forecastdate,mod_type%in%c("Joint_Lik")) |> dplyr::select(`Coefficient`=coef_cfs_mean_ema) |>   
  dplyr::mutate(
