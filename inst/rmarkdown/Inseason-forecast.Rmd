...
# 
#  JF_F<-fit_joint_likelihood(fish_river_ocean ,forecast = 180000,forecast_log_sd = .3)
# 
#  dplyr::bind_rows(sib_R,arima_F,JF_F) |> View()
...
The 10-year (`r paste0((forecast_year-10),"--",(forecast_year-1))` ) average proportion of the run that had passed Bonneville Dam through `r forecastdate  |>  format("%B %d")` is **`r round((Bon_ch_day$Ave_10yr)*100,1)`%**. Based on the cumulative counts to date and this proportion, the expected total dam count is `r format(round(Bon_ch_day$pred_Ave_10yr),scientific=FALSE, big.mark=",")` adult Chinook.
...
#                             mod_result_file = params$mod_result_file)
```
