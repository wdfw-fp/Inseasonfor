...
#fish data
  ...
  Bon_cnts<-data.frame()
num_tries<-0
while((nrow(Bon_cnts)==0)&num_tries<6){
  ...
  if( num_tries==5 & (nrow(Bon_cnts)==0) ){
    stop("Unable to get data ladder split or BDO data")
  }
}
...
num_tries<-0
while((nrow(Fall_split_cnts)==0)&num_tries<6){

  try(Fall_split_cnts<-tule_bright_split(forecastdate))
