...
#   dplyr::group_by(month,md) |> 
#   dplyr::summarise(
#     ave_10yr_flow= mean(cfs_USACoE[Year < forecast_year & Year >= forecast_year - 10], na.rm = TRUE),
#      ave_10yr_temp= mean(cfs_USACoE[Year < forecast_year & Year >= forecast_year - 10], na.rm = TRUE),
#     `Flow (kcfs)`=cfs_USACoE[Year == forecast_year] ,
#     `River temp. (F)`=cfs_USACoE[Year == forecast_year],
#     
#     .groups = "drop"
#   ) |> View()
#   
...
flow_temp_dat2 |>
  ...
  ggplot2::ggplot(ggplot2::aes(x=Date,y=Value,col=type))+ ggplot2::geom_point(size=2.5,show.legend=FALSE)+ggplot2::geom_line(show.legend=FALSE)+ggplot2::facet_wrap(~Param,ncol=1,scales="free_y")+
