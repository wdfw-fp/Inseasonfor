...
while((nrow(Bon_cnts)==0)&num_tries_Bon_cnts<6){

  try(Bon_cnts<-bon_dat_fun(pred_date=NULL,
                      url = "https://www.fpc.org/adults/R_coeadultcount_runsum",
                      past_bon_cnts = past_bon_cnts) |> 
  dplyr::filter(month>=3),silent=TRUE)

  num_tries_Bon_cnts<-num_tries_Bon_cnts+1
  
  # if( num_tries_Bon_cnts==5 & (nrow(Bon_cnts)==0) ){
  #   stop("Unable to get data ladder split data")
  # }
}
...
while((nrow(flow_temp_dat)==0)&num_tries_flow_temp_dat<6){
  flow_temp_dat<-try(get_flow_data(forecastdate = forecastdate,flow_file = past_flow_dat),silent=TRUE) 
  num_tries_flow_temp_dat<-num_tries_flow_temp_dat+1
  
  # if( num_tries_flow_temp_dat==5 & (nrow(flow_temp_dat)==0) ){
...
while((nrow(Fall_split_cnts)==0)&num_tries_Fall_split_cnts<6){

  try(Fall_split_cnts<-tule_bright_split(forecastdate),silent=TRUE)

  num_tries_Fall_split_cnts<-num_tries_Fall_split_cnts+1
  
  # if( num_tries_Fall_split_cnts==10 & (nrow(flow_temp_dat)==0) ){
  #   stop("Unable to get data ladder split data")
  # }
}
